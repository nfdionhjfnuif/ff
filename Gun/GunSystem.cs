...
public class GunSystem : MonoBehaviour
{
    ...
    public void Shoot()
    {
        ...
        if (Physics.Raycast(fpsCam.transform.position, fpsCam.transform.forward, out hit, range))
        {
            ...
            if (hitbox != null)
            {
                Vector3 direction = hit.point - fpsCam.transform.position;
            ...
            if (target != null)
            {
                target.TakeDamage(damage, hit.point - transform.position);
            }
    ...
    void Melee()
    {
        ...
        if (Physics.Raycast(fpsCam.transform.position, fpsCam.transform.forward, out hit, range))
        {
            ...
            if (target != null)
            {
                target.TakeDamage(damage, Vector3.zero); // Pass a default direction value
            }
        }
    }
